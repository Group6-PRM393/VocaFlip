...
import 'package:flutter/material.dart';
import 'create_category_screen.dart'; // Import màn hình thêm mới
import 'edit_category_screen.dart'; // Import màn hình sửa
import 'delete_category_dialog.dart';
...
class _CategoryManagementScreenState extends State<CategoryManagementScreen> {
  // --- Định nghĩa màu sắc ---
  final Color primaryColor = const Color(0xFF135BEC); // Cập nhật màu khớp HTML mới
  ...
  final List<CategoryItem> categories = [
    CategoryItem(id: '1', title: 'Business', subtitle: '12 Decks', icon: Icons.business_center),
    CategoryItem(id: '2', title: 'Travel', subtitle: '5 Decks', icon: Icons.flight),
    CategoryItem(id: '3', title: 'Academic', subtitle: '8 Decks', icon: Icons.school),
    CategoryItem(id: '4', title: 'Daily Life', subtitle: '20 Decks', icon: Icons.local_cafe),
    CategoryItem(id: '5', title: 'Hobbies', subtitle: '3 Decks', icon: Icons.palette),
  ];
 ...
 void _navigateToEdit(CategoryItem category) {
  ...
  void _confirmDelete(CategoryItem category) async {
    ...
    final bool? isDeleted = await showDialog<bool>(
      context: context,
      barrierColor: const Color(0xFF111218).withOpacity(0.6), // bg-[#111218]/60 làm tối màu nền
      builder: (BuildContext context) {
    ...
    if (isDeleted == true) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Đã xóa ${category.title}')),
      );
    }
  }
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(
                    item.title,
                    style: TextStyle(color: textSlate900, fontSize: 16, fontWeight: FontWeight.w600),
                  ),
                  const SizedBox(height: 2),
                  Text(
                    item.subtitle,
            ...
            Row(
              ...
              children: [
                InkWell(
                  ...
                  child: Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                    child: Text('Edit', style: TextStyle(color: primaryColor, fontSize: 14, fontWeight: FontWeight.w600)),
                  ),
                ),
